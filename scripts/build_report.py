...
<svg viewBox="0 0 1000 430" class="svg" role="img" aria-label="System architecture">
  ...
  <rect class="node" x="524" y="148" width="186" height="48" rx="8"/><text class="nt" x="617" y="168" text-anchor="middle">UEBA anomaly</text><text class="ns" x="617" y="184" text-anchor="middle">IsolationForest + z-score</text>
...
<li><b>UEBA.</b> An IsolationForest + z-score model scores how anomalous the host is versus a benign
baseline.</li>
...
<p>UEBA (User &amp; Entity Behaviour Analytics) learns what normal looks like and flags deviations.
Our model is an <b>IsolationForest</b> (an algorithm that isolates outliers) trained on the benign
hosts, combined with a one-sided <b>z-score</b> that flags any single feature sitting far above the
benign mean (for example, a near-perfect beacon whose regularity is ~1.0 when every benign host is
~0). The final anomaly is the stronger of the two — conservative for benign hosts, responsive to a
strong lone signal. This is deliberately the same class of model a commercial UEBA ships; the novelty
is not the anomaly model but what happens next.</p>
...
<pre>src/c2detect/
  ...
  ueba/         IsolationForest + z-score anomaly model (and an OpenUBA adapter)
